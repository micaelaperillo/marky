		...
		<div class="flex-1 min-w-0">
			...
			>
				...
				<span class="inline-flex items-center gap-2">
					<span class="h-2 w-2 rounded-full {running ? 'bg-emerald-500' : 'bg-slate-400'}"
					></span>
					{running ? 'Active' : 'Ended'}
				</span>

				{#if variant === 'latest' && nextUpdateMinutes !== null && nextUpdateMinutes > 0}
					<span class="inline-flex items-center gap-2 text-brand-600 dark:text-brand-400 font-medium">
