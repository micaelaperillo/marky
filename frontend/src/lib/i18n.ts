...
const messages = {
	en: {
		campaign: {
			...
			reportsPlaceholder: 'Sentiment reports will appear here once ingestion runs.',
			statusActive: 'Active',
			statusEnded: 'Ended',
		...
		create: {
			...
			subtitle: 'Pick a name, list the topics you want to track, and set the date range.',
		...
		list: {
			...
			summary: '{count} active {count, plural, one {campaign} other {campaigns}} being monitored.',
	...
	es: {
		campaign: {
			...
			notFoundBody: 'No la pudimos ubicar. Capaz la borraste o el nombre está mal escrito.',
			notFoundCta: 'Volver a campañas',
			notFoundTitle: 'No encontramos esta campaña',
			reportsHeading: 'Reportes',
			reportsPlaceholder: 'Acá van a aparecer los reportes cuando corra la ingesta.',
			statusActive: 'Activa',
			statusEnded: 'Terminó',
		...
		create: {
			...
			subtitle: 'Ponele un nombre, cargá los temas que querés seguir y elegí el rango de fechas.',
		...
		list: {
			...
			summary: '{count} {count, plural, one {campaña activa} other {campañas activas}} en la mira.',
			title: 'Tus campañas',
...
function plural(template: string, vars: Record<string, string | number>): string {
...
function interpolate(template: string, vars?: Record<string, string | number>): string {
	...
	return withPlural.replace(/\{(\w+)\}/g, (_, k) => (vars[k] != null ? String(vars[k]) : `{${k}}`));
}
